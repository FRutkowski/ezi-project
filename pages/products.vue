    ...
    <div class="p-4">
      <Placeholder class="h-48">
        <div class="flex justify-between w-full py-4">
          <div class="flex w-full">Produkt został dodany do koszyka!</div>
          <UButton color="black" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1"
            @click="isAddingProduct = false" />
        </div>
        <p class="py-4 text-sm">Inne produkty sugerowane na podstawie Twojego wyboru:</p>
        <UDivider />
        
        <!-- do usunięcia po dodaniu backendu tylko do testów -->
        <div class="absolute hidden">idx: {{ idx }}</div>
        
        <UTable v-model:columns="columns" :rows="suggestedProduct">
          <template #image-data="{ row }">
            <UButton label="Open" @click="showPhoto(row.photo)" variant="link">
        ...
        <div class="pt-4 w-full flex justify-around">
          <UButton color="gray" label="Kontynuuj zakupy" icon="i-heroicons-banknotes" />
